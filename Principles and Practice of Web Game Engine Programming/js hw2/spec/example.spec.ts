...
describe('Insert Collect', () => {
    ...
    test('Insert Invaild ID', () => {
        const { state } = commander.parse(`lb insert collect -c t1 -d 00X,algorithm,9789866052675`);
        expect(state).toStrictEqual(ProcessState.ERR_ID);
    });

    test('Insert Invaild ISBN', () => {
        const { state } = commander.parse(`lb insert collect -c t1 -d 000,algorithm,9780262033840`);
        expect(state).toStrictEqual(ProcessState.ERR_ISBN);
    });

    test('Insert Duplicate ID', () => {
        const { state } = commander.parse(`lb insert collect -c t1 -d 001,algorithm,9789866052675`);
        expect(state).toStrictEqual(ProcessState.ERR_ID_DUPL);
    });

    test('Insert Duplicate ISBN', () => {
        const { state } = commander.parse(`lb insert collect -c t1 -d 009,algorithm,9780262033848`);
        expect(state).toStrictEqual(ProcessState.ERR_ISBN_DUPL);
    });

    test('Insert into not exists collect', () => {
        const { state } = commander.parse(`lb insert collect -c t0 -d 009,algorithm,9780262033848`);
        expect(state).toStrictEqual(ProcessState.ERR_COLLE);
    });

    test('Insert multi book', () => {
        const { state } = commander.parse(`lb insert collect -c t1 -d 01,book1,9789866052675 -d 02,book2,9789864762859 -d 03,book3,9789865024864`);
...
describe('Update Collect', () => {
    ...
    test('Update with collect value', () => {
        const { state } = commander.parse('lb update collect -c -id 002 -name lisp');
    ...
    test('Update not exist collect', () => {
        const { state } = commander.parse('lb update collect -c t2 -id 002 -name lisp');
        expect(state).toEqual(ProcessState.ERR_COLLE);
    });

    test('Update not exist book', () => {
        const { state } = commander.parse('lb update collect -c t1 -id 010 -name lisp');
...
describe('Delect Book', () => {
    ...
    test('Delete 3 books by name', () => {
        const { state } = commander.parse(`lb delete collect -c t1 -ids 002 -names learn-js,algorithm`);
        const { books } = readData().t1;
        expect(state).toEqual(ProcessState.SUCCESS);
    ...
    test('Delete Witout collect', () => {
        const { state } = commander.parse('lb delete collect -ids 2 -names n1,n2,n3');
        expect(state).toEqual(ProcessState.ERR_ARGS_COLLE);
    });
...
describe('Import Data', () => {
    ...
    test('Import not exists file', () => {
        const { state } = commander.parse(`lb import collect -f ${join(__dirname, 'empty.json')}`);
